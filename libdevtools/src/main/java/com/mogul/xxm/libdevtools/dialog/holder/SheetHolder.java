package com.mogul.xxm.libdevtools.dialog.holder;
...
import com.mogul.xxm.libdevtools.dialog.adapter.TieAdapter;
import com.mogul.xxm.libdevtools.dialog.listener.OnItemClickListener;
    ...
    public void assingDatasAndEvents(final Context context, final BuildBean bean) {
        ...
        if (bean.isVertical) {
           ...
           // rView.addItemDecoration(new DividerItemDecoration(bean.mContext, ToolUtils.dip2px(bean.mContext, 8), true, Color.TRANSPARENT, true, true, false));
            rView.addItemDecoration(new DividerItemDecoration(bean.mContext, DividerItemDecoration.VERTICAL));
        } else {
            rView.setLayoutManager(new GridLayoutManager(bean.mContext, bean.gridColumns));// 布局管理器。
            rView.addItemDecoration(new DividerItemDecoration(bean.mContext, DividerItemDecoration.HORIZONTAL));
        }
